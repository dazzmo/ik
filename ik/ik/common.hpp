...
typedef vector_tpl_t<number_t> vector_t;
typedef vector_tpl_t<number_t, 3> vector3_t;

typedef Eigen::Ref<vector_t> vector_ref_t;
typedef Eigen::Ref<const vector_t> vector_const_ref_t;

typedef matrix_tpl_t<number_t> matrix_t;
typedef matrix_tpl_t<number_t, 3, 3> matrix3_t;
...
struct default_solver_parameters {
    ...
    number_t max_time = 1.0;
    // Step length to perform
    number_t step_length = 1.0;
};
