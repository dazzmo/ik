...
class GravityCost : public Cost {
   public:
     ...
     */
    GravityCost(const model_t &model, const index_t &nj) : Task(), nj_(nj) {}

    /**
     * @brief Factory method to create a shared pointer to a posture cost.
     *
     * @param model The Pinocchio model of the robot.
     * @return A shared pointer to the created `GravityCost` instance.
     */
    static std::shared_ptr<GravityCost> create(const model_t &model,
                                               const index_t &nj) {
        return std::make_shared<GravityCost>(model, nj);
    }
