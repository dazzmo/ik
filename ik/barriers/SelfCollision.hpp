...
class SelfCollisionBarrier : public BarrierAbstract {
   public:
    ...
    void computeBarrier(const Configuration &cfg, Eigen::Ref<Vector> barrier) {
        ...
        std::sort(pair_distance_.begin(), pair_distance_.end(),
                  [](const std::pair<Size, Real> &a,
                     const std::pair<Size, Real> &b) -> bool {
    ...
    void computeJacobian(const Configuration &cfg,
        ...
        for (const Size &idx : closest_indices_) {
            ...
            // compute row
            jacobian.row(row) =
                n.transpose() * J1.topRows(3) + r1.cross(n).transpose() * J1.bottomRows(3);
            jacobian.row(row) -=
                n.transpose() * J2.topRows(3) + r2.cross(n).transpose() * J2.bottomRows(3);
            row++;
        }
    }
